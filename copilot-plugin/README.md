# oc-notifier Copilot CLI plugin
...
```
Copilot CLI hooks  →  forward.sh  →  oc-notifier HTTP ingest  →  providers
  agentStop                       POST /v1/copilot-cli/hook
  notification (permission_prompt
                | elicitation)
```

| Copilot event | Matcher | oc-notifier type | Notes |
|---------------|---------|------------------|-------|
...
# → installs scripts under ~/.copilot/hooks/oc-notifier
...
ln -s /path/to/oc-notifier/copilot-plugin ~/.copilot/hooks/oc-notifier
```

Example `~/.copilot/hooks/oc-notifier.json`:
...
```json
{
  ...
  "hooks": {
    "agentStop": [
      {
        "type": "command",
        "bash": "bash /home/YOU/.copilot/hooks/oc-notifier/scripts/forward.sh",
        "powershell": "bash /home/YOU/.copilot/hooks/oc-notifier/scripts/forward.sh",
        "timeoutSec": 15
      }
    ],
    "notification": [
...
echo '{
  ...
  "stopReason": "end_turn"
}' | curl -sS -X POST http://127.0.0.1:4100/v1/copilot-cli/hook \
