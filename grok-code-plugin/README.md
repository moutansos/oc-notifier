# oc-notifier Grok Code plugin

Thin Grok Build plugin that forwards Stop, Notification, and PermissionDenied
hooks to [oc-notifier](https://code.msyke.dev/mSyke/oc-notifier). **oc-notifier
owns notification routing**; this plugin only relays hook payloads.

## How it works

```
Grok hooks  →  forward.sh  →  oc-notifier HTTP ingest  →  providers
...
# → ~/.grok/plugins/oc-notifier (auto-trusted)
```

Optional env: `OC_NOTIFIER_URL` (default `http://127.0.0.1:4100`), `OC_NOTIFIER_TOKEN`.
...
echo '{"sessionId":"t","cwd":"/tmp","hookEventName":"stop","reason":"end_turn"}' \
  | curl -sS -X POST http://127.0.0.1:4100/v1/grok-code/hook -H 'Content-Type: application/json' -d @-
